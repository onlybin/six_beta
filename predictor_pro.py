...
def predict_next_period(db_file='lottery.db', output_file='prediction.json', memory_file='learning_memory.json'):
    ...
    for i in range(len(reversed_records) - 1):
        ...
        for past_draw in reversed(reversed_records[:i+1]):
            if NUM_TO_COLOR.get(past_draw['special'], '绿') == last_special_color:
                ...
                break
        recent_5_wuxings = [NUM_TO_WUXING.get(r['special'], '金') for r in reversed_records[max(0, i-4):i+1]]
    ...
    recent_5_wuxings = [NUM_TO_WUXING.get(r['special'], '金') for r in records[:5]]
    ...
    print(f"\n>>> 12维降阶全引擎推演完毕 - 第 {next_period} 期高分特码雷达 (前6名观测):")
    top6_specials = []
